...
def generate_draft():
    ...
    if draft_type not in DRAFT_TYPES:
        return jsonify({
            "error": f"type must be one of {DRAFT_TYPES}"
        }), 400
    ...
    previous_drafts = [row_to_draft(r) for r in previous_rows]

    contents, provider = generate_with_gemini(
        application,
        draft_type,
        previous_drafts
    )
    ...
    result = row_to_draft(new_row)
    result["provider"] = provider

    return jsonify(result), 201


@app.get("/api/applications/<app_id>/drafts")
def list_drafts(app_id):
    conn = get_db()
    ...
    return jsonify([row_to_draft(row) for row in rows])
...
def update_draft(draft_id):
    ...
    contents = data.get("contents", current["contents"])
    ...
    return jsonify(row_to_draft(updated))


@app.get("/api/applications/<app_id>/status-history")
def status_history(app_id):
    conn = get_db()
